...
class Checkbox extends Widget {
    ...
    constructor(parent: Window) {
        ...
        console.log("[Checkbox] Component initialized with label:", this._input);
    }
    ...
    private drawCheckmark(): void {
        ...
        if (this._isChecked) {
            ...
            console.log("[Checkbox] Checkmark drawn - state is checked.");
        } else {
            console.log("[Checkbox] Checkmark removed - state is unchecked.");
        }
    }
    ...
    set label(value: string) {
        ...
        if (this._text) {
            ...
            console.log("[Checkbox] Label updated to:", value);
        }
    }
    ...
    render(): void {
        ...
        let eventRect = this._group.rect(this.width, this.height).opacity(0).attr('id', 2);
        this.registerEvent(eventRect);

        eventRect.on("click", () => {
            this._isChecked = !this._isChecked;
            console.log(`[Checkbox] Checkbox clicked. New state: ${this._isChecked ? 'checked' : 'unchecked'}`);
            this.drawCheckmark();

            if (this._onToggleHandler) {
                console.log("[Checkbox] onToggleHandler triggered.");
                this._onToggleHandler(this._isChecked);
            }

            if (this._onClickHandler) {
                console.log("[Checkbox] onClickHandler triggered.");
    ...
    onClick(callback: () => void): void {
        ...
        console.log("[Checkbox] onClick callback registered.");
    }

    onToggle(callback: (checked: boolean) => void): void {
        this._onToggleHandler = callback;
        console.log("[Checkbox] onToggle callback registered.");
    }
